...
##*** Metodos de la clase ***************************************************
    ...
    def buscarPlan(self):
        try:
            ...
            codplan= %s"""%(self.plan),dbparams.dbname,dbparams.dbuser,dbparams.dbpass)
    ...
    def CrearAfiliacion (self):
        try:            
            ...
            conexion = database.operacion("","""SELECT * FROM PRODUCTO WHERE 
                    numserie= '%s'"""%(self.producto),dbparams.dbname,dbparams.dbuser,dbparams.dbpass)
            resultado = conexion.execute()
    ...
    def CrearContratacion (self):
        try:
            ...
            conexion = database.operacion("","""SELECT codpaq FROM PAQUETE 
                WHERE codpaq = %s"""%(self.plan),dbparams.dbname,
...
con el paquete de codigo %s \n""")%(self.producto, self.plan)
    ...
    def desafiliarContratacion(self):
        try:
            ...
            """%(self.plan,self.producto),dbparams.dbname,dbparams.dbuser,dbparams.dbpass)
            resultado = conexion.execute()
