 ...
 * Un plan que incluye un servicio debe tener una cantidad no nula y una tarifa
 * nula si es
 * postpago. Debe tener una cantidad nula y una tarifa no nula si es prepago.
 */
...
--
-- Procedimientos para manejar las consultas de la tarea 2
-- 
...
DECLARE
  ...
  servicioIncluido INTEGER;
  consumo CONSUME%ROWTYPE;
  diferencia INTEGER;
  tarifaIncluida REAL;
  cantidadConsumida INTEGER;
  sum REAL;  
BEGIN
  ...
  LOOP
    ...
    IF EXISTS (SELECT * 
               FROM ACTIVA 
               WHERE numserie = serialProducto) THEN
    
      codigoPlan = (SELECT codPlan FROM ACTIVA WHERE numserie = serialProducto);

      FOR codigoServicio IN SELECT codserv FROM INCLUYE WHERE codplan = codigoPlan
      LOOP
        
        cantidadConsumida := 0;
        servicioIncluido := (SELECT cantidad FROM INCLUYE WHERE codplan = codigoPlan AND codserv = codigoServicio);
        tarifaIncluida := (SELECT tarifa FROM INCLUYE WHERE codplan = codigoPlan AND codserv = codigoServicio);
        
        FOR consumo IN SELECT * FROM CONSUME WHERE numserie = serialProducto AND codserv = codigoServicio
          AND to_char(fecha,'MM') = to_char(CURRENT_DATE,'MM')
          AND to_char(fecha,'YYYY') = to_char(CURRENT_DATE,'YYYY')
        LOOP
          cantidadConsumida := cantidadConsumida + consumo.cantidad;
        END LOOP;
        
        diferencia := servicioIncluido - cantidadConsumida;
        IF (diferencia > 0) THEN
          sum := sum + diferencia*tarifaIncluida;
        END IF;
